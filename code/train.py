...
print(os.path.exists('../../../data/model_checkpoints'))
...
normalized_data = torch.load('../../../data/normalized_train.pt')
...
train_loader = DataLoader(train_dataset, batch_size=2,shuffle=False, generator=torch.Generator(device='cpu'))
...
for epoch in range(epochs):
    ...
    for i, (upper_air, target_surface) in enumerate(train_loader):    
        ...
        output_surface = output_surface.squeeze(1)
    ...
    if (epoch + 1) % 10 == 0:
        checkpoint_path = f'../../../data/model_checkpoints/model_epoch_{epoch+1}.pth'
        torch.save({
            'epoch': epoch + 1,
...
torch.save(model, '../../../data/3DCNN_0005_100ep.pth')